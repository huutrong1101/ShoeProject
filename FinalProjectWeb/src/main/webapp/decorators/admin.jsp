...
.chooseShop:hover .noidung_dropdown {
	display: block;
}
...
<body>
	<!-- header  -->
	<%@ include file="/common/admin/leftAdmin.jsp"%>
	<!-- end header  -->
...
function invoiceDetailAdmin(invoiceId) {
	$.ajax({
		...
		success : function(data) {
			var row = document.getElementById("contentInvoiceAdmin");
			row.innerHTML = data;
		},
		error : function(xhr) {
...
function detailRequest(requestID) {
	$.ajax({
		url : "/FinalProjectWeb/ajax/detailRequest",
		type : "get", //send it through get method
		data : {
			requestID : requestID
		},
		success : function(data) {
			var row = document.getElementById("contentRequest");
			row.innerHTML = data;
		},
		error : function(xhr) {
			//Do Something to handle error
