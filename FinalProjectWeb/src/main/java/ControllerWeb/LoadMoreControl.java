...
@WebServlet(name = "LoadMoreControl", urlPatterns = { "/ajax/load" })
public class LoadMoreControl extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// tam thoi load ra 4 san pham truoc
		String amount = request.getParameter("exits");
		int iamount = Integer.parseInt(amount);

		ProductServices productServices = new ProductServicesImpl();
		List<Product> list = productServices.getNext4Product(iamount);
		...
		for (Product o : list) {
			...
			out.println("<div class=\"product col-lg-3 col-md-6\">\r\n"
					+ "							<div class=\"single-product\">\r\n"
					+ "								<c:choose>\r\n"
					+ "									<c:when test=\"${o.image.substring(0,4) == 'http' }\">\r\n"
					+ "										<a href=\"detail?pid=" + o.getId()
					+ "\" title=\"View Product\"> <img\r\n"
					+ "											class=\"img-fluid\" src=\"" + o.getImage()
					+ "\" alt=\"\" />\r\n" + "										</a>\r\n"
					+ "									</c:when>\r\n"
					+ "									<c:otherwise>\r\n"
					+ "										<c:url value=\"/image?fname=" + o.getImage()
					+ "\" var=\"imgUrl\"></c:url>\r\n" + "										<a href=\"detail?pid="
					+ o.getId() + "\" title=\"View Product\"> <img\r\n"
